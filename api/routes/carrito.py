...
carrito_routes = Blueprint("carrito_routes", __name__)


@carrito_routes.route('/',methods=['GET'])
def hola():
    return jsonify({'message':"Prueba de funcionamiento....... HOLA!"})

#Metodo para añadir Es de Ejemplo para el proyecto
@carrito_routes.route('/create_user',methods=['POST'])
def create_user():
    """print(request)
    print(dir(request))"""
...
@carrito_routes.route('/obtener_venta',methods=['GET'])
def obtener_venta():
    data = json.loads(request.data)
    print(data)
    if 'username' not in data:
...
@carrito_routes.route('/ventas', methods=['GET'])
def ventas():
    with engine.connect() as con:
...
@carrito_routes.route('/ventas', methods=["PUT"])
def cambiar_venta():
    data = json.loads(request.data)
    if 'id' not in data:
   ...
   #Eliminar 
@carrito_routes.route('/ventas',methods=['DELETE']) 
def eliminar_venta():
    data = json.loads(request.data)
    if 'id' not in data:
        return jsonify({"Respuesta":"Id no enviado para su consulta"})
